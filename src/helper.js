...
function show_barcode() {
    ...
    var barcode = cleanStr(document.forms.decode_form.barcode.value);
...
export function do_encode(data) {

    if((Object.keys(data).length === 0) || (Object.values(data).filter(a => a !== '').length ===0)){
        alert('No input provided');
        return null;
    } 
  

    var message = check_fields(data);
...
export function do_update() {
    var len = show_barcode();
    if (len == 65) do_decode();
}


export const showBarcode = (barcode) => {
    ...
    for (let i = 0; i < 3; i++) {
        // Create an array to hold cells within each row
        const cells = [];

        // Loop to generate cells within each row
        for (let j = 0; j < 129; j++) {
            // Create a table data cell element and push it to the cells array
            cells.push(<td key={j} style={{ width: '2px', height: '8px' }}></td>);
        }

        // Create a table row element and push it to the rows array
        rows.push(<tr key={i} id={'row' + i}>{cells}</tr>);
    }
    console.log(rows, 'kkkkkkkkk', rows[0]);
    // Process the barcode string
    const cleanedBarcode = cleanStr(barcode || '');
    const len = Math.min(cleanedBarcode.length, 65);

    console.log(cleanedBarcode, 'manushahi')

    // Rendering logic
    for (let i = 0; i < len; i++) {
        let j = 2 * i;
        switch (cleanedBarcode[i]) {
            case 'A':
                topRow.push(<td key={j} style={{ width: '2px', height: '8px' }}></td>);
                midRow.push(<td key={j} style={{ width: '2px', height: '8px' }}></td>);
                btmRow.push(<td key={j} style={{ width: '2px', height: '8px' }}></td>);
                topRow.push(<td key={j} style={{ backgroundColor: '#000', width: '2px', height: '8px' }}></td>);
                midRow.push(<td key={j} style={{ backgroundColor: '#000', width: '2px', height: '8px' }}></td>);
                btmRow.push(<td key={j} style={{ backgroundColor: 'transparent', width: '2px', height: '8px' }}></td>);
                break;
            case 'D':
                topRow.push(<td key={j} style={{ width: '2px', height: '8px' }}></td>);
                midRow.push(<td key={j} style={{ width: '2px', height: '8px' }}></td>);
                btmRow.push(<td key={j} style={{ width: '2px', height: '8px' }}></td>);
                topRow.push(<td key={j} style={{ backgroundColor: 'transparent', width: '2px', height: '8px' }}></td>);
                midRow.push(<td key={j} style={{ backgroundColor: '#000', width: '2px', height: '8px' }}></td>);
                btmRow.push(<td key={j} style={{ backgroundColor: '#000', width: '2px', height: '8px' }}></td>);
                break;
            case 'F':
                topRow.push(<td key={j} style={{ width: '2px', height: '8px' }}></td>);
                midRow.push(<td key={j} style={{ width: '2px', height: '8px' }}></td>);
            ...
            default:
                ...
                midRow.push(<td key={j} style={{ width: '2px', height: '8px' }}></td>);
                btmRow.push(<td key={j} style={{ width: '2px', height: '8px' }}></td>);
                topRow.push(<td key={j} style={{ backgroundColor: '#f00', width: '2px', height: '8px' }}></td>);
                midRow.push(<td key={j} style={{ backgroundColor: '#f00', width: '2px', height: '8px' }}></td>);
                btmRow.push(<td key={j} style={{ backgroundColor: '#f00', width: '2px', height: '8px' }}></td>);
                break;
        }
    }

    // Pad the barcode if needed
    for (let i = len; i < 65; i++) {
        topRow.push(<td key={i} style={{ backgroundColor: 'transparent', width: '2px', height: '8px' }}></td>);
        midRow.push(<td key={i} style={{ backgroundColor: '#ccc', width: '2px', height: '8px' }}></td>);
        btmRow.push(<td key={i} style={{ backgroundColor: 'transparent', width: '2px', height: '8px' }}></td>);
    }

    return { topRow: topRow, midRow: midRow, btmRow: btmRow }
